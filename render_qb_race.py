...
OUT_MP4  = "qb_passing_yards_race_2025_youtube.mp4"
...
def _load_png_array(path: str, max_px: int):
...
def update(idx):
    ...
    # ✅ Title centered using axes coords (never drifts)
    ax.text(
        0.5, 1.045,
    ...
    # Week badge centered (also axes coords = stable)
    ax.text(
        0.5, 0.99,
    ...
    for i, (player, val) in enumerate(zip(names, vals)):
        ...
        # Team logo (sharper)
        cached_logo = get_team_logo(team)
